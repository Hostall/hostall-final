...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Initialize hostel manager for the new system
  if (window.hostelManager) {
    console.log('🏠 Initializing hostel manager...');
    window.hostelManager.init();
  }
  ...
  setTimeout(() => {
    if (document.getElementById('public-list')) {
      ...
      // Force hostel manager refresh
      if (window.hostelManager) {
        window.hostelManager.loadHostels().then(() => {
